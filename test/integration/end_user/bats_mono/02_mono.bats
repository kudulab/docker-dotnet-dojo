...
@test "mono-runtime is installed and mono is invocable" {
  run /bin/bash -c "ide --idefile Idefile.to_be_tested \"dpkg -s mono-runtime && mono --version\""
...
@test "nunit-console is installed and invocable" {
  run /bin/bash -c "ide --idefile Idefile.to_be_tested \"dpkg -s nunit-console && nunit-console --version\""
  assert_output --partial "NUnit-Console"
  assert_equal "$status" 0
}
@test "nuget is invocable" {
  run /bin/bash -c "ide --idefile Idefile.to_be_tested \"nuget help\""
