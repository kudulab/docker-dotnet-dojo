...
@test "msbuild is invocable" {
  run /bin/bash -c "ide --idefile Idefile.to_be_tested \"msbuild /version\""
  assert_output --partial "Microsoft (R) Build Engine"
  assert_equal "$status" 0
}
@test "dotnet is invocable" {
  run /bin/bash -c "ide --idefile Idefile.to_be_tested \"dotnet --info\""
  assert_output --partial "Microsoft.NETCore.App 2.1.4"
  assert_equal "$status" 0
}
@test "dotnet core projects can be built" {
  run /bin/bash -c "ide --idefile Idefile.to_be_tested \"\
...
@test "DOTNET_CLI_TELEMETRY_OPTOUT is set" {
  run /bin/bash -c "ide --idefile Idefile.to_be_tested \"env | grep DOTNET\""
  assert_output --partial "DOTNET_CLI_TELEMETRY_OPTOUT=1"
  assert_equal "$status" 0
}
@test "paket is invocable" {
  run /bin/bash -c "ide --idefile Idefile.to_be_tested \"paket --version\""
  assert_output --partial "Paket version 5.182.0-alpha001"
  assert_equal "$status" 0
}
@test "fake is invocable" {
  run /bin/bash -c "ide --idefile Idefile.to_be_tested \"fake --version\""
